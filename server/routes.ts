...
export async function registerRoutes(
  ...
  app.get(api.sheets.list.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const sheets = await storage.getSheets(req.user.id);
    res.json(sheets);
  });
  ...
  app.get(api.sheets.get.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
  ...
  app.delete(api.sheets.delete.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const sheet = await storage.getSheet(Number(req.params.id));
    if (!sheet) return res.status(404).json({ message: "Sheet not found" });
    if (sheet.userId !== req.user.id) return res.sendStatus(403);
  ...
  app.get(api.problems.list.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    const sheet = await storage.getSheet(Number(req.params.id));
    if (!sheet) return res.status(404).json({ message: "Sheet not found" });
    if (sheet.userId !== req.user.id) return res.sendStatus(403);
  ...
  app.put(api.problems.update.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    try {
  ...
  app.delete(api.problems.delete.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    await storage.deleteProblem(Number(req.params.id));
    res.sendStatus(204);
  });

  app.get(api.stats.dashboard.path, async (req: AuthenticatedRequest, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
